...
class CombinedOrderedEnumerator < Enumerator
  ...
  # it will reference the error enumerator
  ...
  # initilise the CombinedOrderedEnumerator
  # can be given a number of enumerators
  def initialize(*args)
    @args = args
  end
  ...
  def take(n)
    # check here
    out = Array.new
    # iterate every enumerator
    # put all vaues into an array
    # then sort the array in ascending
    # emit the first N element
    @args.each do |arg|
      arg_array = arg.take(n).to_a
      # raise exception when the given enumerator does not in ascending order
      raise UnorderedEnumerator.new(arg), 'Enumerator must in ascending order' unless arg_array == arg_array.sort
      out << arg_array
    end
    out = out.flatten.sort
    out.first([n, out.length].min)
  end
end
